...
#' @return a dataframe containing the disctinct values of the groups of df, and for each group a mutual information column (I). If df was not grouped this will be a single entry
#' @import dplyr
calculateDiscreteContinuousMI_SGolay = function(df, discreteVar, continuousVar, k_05=10) {
  grps = df %>% groups()
  ...
  tmp2 = tmp %>%  group_by(!!!grps,x.discrete) %>% arrange(y.continuous) %>% group_modify(
    function(d,...) {
      tibble(
        N_X=d$N_X,
        N = d$N,
        y.continuous = d$y.continuous,
        d_xy_d_r = tryCatch(
          signal::sgolayfilt(d$y.continuous, p=2, n=k_05*2+1, m=1, ts=1/mean(d$N_X)), 
          error = function(e) {
            print("sgolay method threw an error")
            rep(NA,length(d$y.continuous)) 
          }
        )
      ) %>% mutate(
  ...
  tmp2 = tmp2 %>%  group_by(!!!grps) %>% arrange(y.continuous) %>% group_modify(
    function(d,...) {
      tibble(
        N_X=d$N_X,
        N = d$N,
        x.discrete = d$x.discrete,
        y.continuous = d$y.continuous,
        p_y_given_x = d$p_y_given_x,
        d_xy_d_r = d$d_xy_d_r,
        d_y_d_r = tryCatch(
          signal::sgolayfilt(d$y.continuous,p=2,n=k_05*2+1,m=1,ts=1/(mean(d$N))),
          error = function(e) {
            print("sgolay method threw an error")
            rep(NA,length(d$y.continuous))
          }
  ...
  tmp3 = tmp2 %>% group_by(!!!grps,x.discrete) %>% arrange(y.continuous) %>% mutate(
    # integrate over dy ( well does the trapeziod part of the integration
    d_I_d_xy = (pmi_xy+lag(pmi_xy,1,default=0))*(y.continuous-lag(y.continuous))/2
...
calculateDiscreteContinuousMI_Discretise = function(df, discreteVar, continuousVar, bins=nrow(df)^(1/3)) {
  ...
  #      d = d %>% mutate(x.discrete = as.integer(as.factor(!!discreteVar))) %>% select(x.discrete, y.continuous = !!continuousVar) %>% infotheo::discretize(n=bins)
  #     return(data.frame(I=infotheo::mutinformation(d$x.discrete, d$y.continuous)))
  # })
  #)
}
