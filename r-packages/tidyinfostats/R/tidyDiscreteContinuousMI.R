...
#' @param method - the method employed - valid options are "KWindow","KNN","SGolay","DiscreteByRank","DiscreteByValue","Compress","Entropy". At the moment "SGolay" not supported for dbplyr tables
...
#' @param groupVar - the column of the categorical value (X)
#' @param valueVar - the column of the continuous value (Y)
#' @param bytes - the number of bytes to use for pre compression encoding of continuous variables
#' @return a dataframe containing the disctinct values of the groups of df, and for each group a mutual information column (I). If df was not grouped this will be a single entry
#' @import dplyr
calculateDiscreteContinuousMI_Compression = function(df, discreteVar, continuousVar, ...) {
  grps = df %>% groups()
  discreteVar = ensym(discreteVar)
  continuousVar = ensym(continuousVar)
  ...
  # https://arxiv.org/abs/1709.10164
  # Universal and accessible entropy estimation using a compression algorithm Ram Avinery, Micha Kornreich, Roy Beck
