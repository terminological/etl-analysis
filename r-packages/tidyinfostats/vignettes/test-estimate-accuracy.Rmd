...
experiment1 = function(distribution, sampleSize, reps) {
  ...
  for (i in c(1:reps)) {
  	...
  	kout = kout %>% bind_rows(tibble(
  			...
  			value = k,
  			estimatedMI = kresult,
  			estimatedMean = rep(estMean,length(k)),
  			estimatedSd = rep(estSd,length(k))
  	))
...
plotExperiment1 = function(df) {
  exp1aDataGrouped = df %>% group_by(method,param,value) %>% summarise( #,estimatedMean,estimatedSd,theoreticalMI,theoreticalMean,theoreticalSd) %>% summarise(
    estMIMean = mean(estimatedMI),
    estMIsd = sd(estimatedMI)
  )
...
*** solve estimated versus theoretical for parameters a&b for random distributions & samples
...
```{r}
df = exp2data %>% mutate(error = ((theoretical-estimated)/theoretical), components=as.factor(distributions))
